...
class LedgerRepository {
  ...
  LedgerRepository(this.apiService);

  Future<JSON> fetch(String token, String endpoint) async {
    var response = await apiService.get(endpoint: ApiConstants.ledgerAPI+endpoint, token: token);
    return response;
  }

  Future<JSON> fetchByColumn(String token, String endpoint, Map<String, String> data) async {
    var response = await apiService.post(endpoint: ApiConstants.ledgerAPI+endpoint, token: token, body: data);
    return response;
  }
}
