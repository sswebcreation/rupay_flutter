...
class SinglePurchasePDFController extends GetxController {
  SinglePurchasePDFController();

  final storage = GetStorage();

  final PurchaseProvider purchaseProvider = Get.find();
  ...
  void getPurchaseByID() {
    purchaseProvider.singleFetch(storage.read("access"), entryType+id.toString()).then((response) async {
  ...
  void getPurchase() {
    ...
    purchaseProvider.fetchByColumn(storage.read("access"), ApiConstants.multiple, data).then((response) async {
      if(response.code==1) {
  ...
  Widget getHeader(PurchaseModel purchase) {
    return Padding(
  ...
  Widget getFooter(PurchaseModel purchase) {
    return Padding(
  ...
  Widget getRightBillDetails(PurchaseModel purchase) {
    return Container(
      ...
      child: Column(
          ...
          mainAxisSize: MainAxisSize.max,
          children: [
            getInfoCombo("Invoice No.", (purchase.PARTYBILLNO??"").trim()),
            SizedBox(
              height: 2
            ),
            getInfoCombo("Invoice Date", purchase.PARTYBILLDATE!=null ? DateFormat("dd-MM-yyyy").format(DateTime.parse(purchase.PARTYBILLDATE??"")) : ""),
          ]
      )
    );
  }
