...
class PurchaseController extends GetxController {
  ...
  void getPurchase() {
    ...
    purchaseProvider.fetchByColumn(storage.read("access"), type+ApiConstants.filter, data).then((response) async {
  ...
  void searchPurchase() {
    ...
    if(search.text.isEmpty) {
      ...
      purchases_temp = [];
      for (var element in purchases) {
        if((element.PARTYBILLNO??"").trim().toLowerCase() == search.text.toLowerCase()  || (element.ACCOUNTNAME??"").trim().toLowerCase().contains(search.text.toLowerCase())) {
          if(status.isNotEmpty) {
            if (status.contains(element.PendingStatus) || (status.contains("PART PENDING") && (element.PendingStatus ?? "").contains("PART PENDING"))) {
              purchases_temp.add(element);
              amount += element.NETAMOUNT??0;
  ...
  void choosePDFType(BuildContext context) {
    if(selected_purchases.isEmpty) {
      ...
      goto("/singlePurchasePDF", arguments: {"selected" : selected_purchases, "type" : "M", "entryType" : type});
    }
  }
  ...
  void delete() {
    ...
    purchaseProvider.deleteMultiple(ApiConstants.delete, storage.read("access"), data).then((response) async {
      print(response.toJson());
  ...
  String getPurchaseIDs() {
    String id = "";
    for (var element in selected_purchases) {
      id+=",${element.ID}";
    }
