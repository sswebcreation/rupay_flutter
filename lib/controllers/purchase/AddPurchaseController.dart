...
class AddPurchaseController extends GetxController {
  ...
  late PurchaseProvider purchaseProvider = Get.find();
  ...
  void getValue() {
    ...
    purchaseProvider.fetchValue(storage.read("access"), ApiConstants.value).then((response) async {
      if(response.code==1) {
  ...
  void addPurchase() {
    if(formKey.currentState!.validate()) {
      ...
      purchaseProvider.add(storage.read("access"), ApiConstants.add, data).then((
          response) async {
