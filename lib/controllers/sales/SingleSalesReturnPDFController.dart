...
class SingleSalesReturnPDFController extends GetxController {
  SingleSalesReturnPDFController();

  final storage = GetStorage();

  final SalesReturnProvider salesReturnProvider = Get.find();
  ...
  void getSalesByID() {
    salesReturnProvider.singleFetch(storage.read("access"), id.toString()).then((response) async {
      if(response.code==1) {
        sales = [];
  ...
  void getSales() {
    ...
    salesReturnProvider.fetchByColumn(storage.read("access"), ApiConstants.multiple, data).then((response) async {
      if(response.code==1) {
        sales = response.data??[];
  ...
  processPDF() async {
    if (await Essential.requestPermission().isGranted) {
      ...
      String name = "US-Sales-${sales.length==1 ? sales.first.DOCNUMBER.toString().trim() : DateFormat("dd-MM-yyyy_hh-mm-ss").format(DateTime.now())}";
  ...
  Widget getRightBillDetails(SalesModel sales) {
    return Container(
      ...
      child: Column(
          ...
          mainAxisSize: MainAxisSize.max,
          children: [
            getInfoCombo("Note No.", (sales.DOCNUMBER??"").trim()),
            SizedBox(
              height: 2
            ),
            getInfoCombo("Note Date", DateFormat("dd-MM-yyyy").format(DateTime.parse(sales.TXNDATE??""))),
            SizedBox(
                height: 2
            ),
            getInfoCombo("Original Bill No.", (sales.ORIGINALBILLNO??"").trim()),
            SizedBox(
                height: 2
            ),
            getInfoCombo("Original Bill Date", (sales.ORIGINALBILLDT??"").isNotEmpty ? DateFormat("dd-MM-yyyy").format(DateTime.parse(sales.ORIGINALBILLDT??"")) : ""),
