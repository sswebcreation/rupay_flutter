...
class MultiLedgerController extends GetxController {
  MultiLedgerController();
    ...
    ledgerProvider.fetchByColumn(storage.read("access"), ApiConstants.filter, data).then((response) async {
      if(response.code==1) {
        ...
        if(customers.length<=1) {
          ...
          //     if(i==0) {
          //       ledgers.add(element);
          //       if(element.DOCNUMBER.toString().trim()=="0") {
          //         element = element.copyWith(DAYBOOKNAME: "Opening Balance");
          //       }
          //       else {
          //         ledgers.insert(0, LedgerModel(Particulars: "Opening Balance", DOCNUMBER: "0", Amount: 0, ACCOUNTCODE: element.ACCOUNTCODE, ACCOUNTNAME: element.ACCOUNTNAME, DAYBOOKNAME: "Opening Balance", ADD1: element.ADD1, ADD2: element.ADD2));
          //       }
          //     }
          //     else {
          //       if(element.DOCNUMBER.toString().trim()!="0") {
          ...
          // else {
            ...
            for (int i = 0; i < leds.length; i++) {
              ...
              if (date.difference(sd).isNegative) {
                ...
                print("hello");
                for(int j = i; j<leds.length; j++) {
                  if(leds[j].DOCNUMBER.toString().trim()!="0") {
                    ledgers.add(leds[j]);
                  }
                }
                // ledgers = leds.sublist(i + 1);
                if(leds[0].DOCNUMBER.toString().trim()!="0") {
                  print(leds.length);
                  print(leds[0]);
                  print(i);
                  print("iiiiii");
                  ...
                  if(index==0) {
                    ledgers.insert(0, LedgerModel(Particulars: "Opening Balance",
                        DOCNUMBER: "0",
                        Amount: opening,
                        ACCOUNTCODE: leds[index].ACCOUNTCODE,
                        ACCOUNTNAME: leds[index].ACCOUNTNAME,
                        DAYBOOKNAME: "Opening Balance",
                        ADD1: leds[index].ADD1,
                        ADD2: leds[index].ADD2));
                  }
                  else {
                  ...
                  leds[0] = leds[0].copyWith(DAYBOOKNAME: "Opening Balance");
                  ledgers.insert(0, leds[0]);
                }
                break;
              }
            }
          ...
          for (int i = 0; i < leds.length; i++) {
            ...
            if(i==0) {
              ...
              print(leds[last].DOCNUMBER);
              print("leds[last].DOCNUMBER");
              print(leds[last].DOCNUMBER.toString().trim()!="0");
              ...
              for(int j = last; j<(i); j++) {
                ...
                if(leds[j].DOCNUMBER.toString().trim()!="0" || (leds[j].DOCNUMBER.toString().trim()=="0" && cnt==0)) {
                  ledgers.add(leds[j]);
                }
                cnt++;
              }
          ...
          for(int j = last; j<leds.length; j++) {
            if(leds[j].DOCNUMBER.toString().trim()!="0") {
              ledgers.add(leds[j]);
            }
          }
  ...
  goto(String path, {dynamic arguments}) {
    Get.toNamed(path, arguments: arguments)?.then((value) {
      if(value!=null) {
        ...
        if(arguments['title'].contains("Companies")) {
          companies = value;
        }
        update();
      }
  ...
  String getCompanyString() {
    ...
    return where.isNotEmpty ? " led.COCODE IN (${where.substring(2)})" : "";
  }
  ...
  List<CompanyModel> getCompanyList(int index) {
    if(index==0) {
      return companies;
    }
    else {
