...
class AddBankController extends GetxController {
  ...
  late BankProvider bankProvider = Get.find();
  ...
  late TextEditingController chequeDate;
  late TextEditingController chequeNo;
  ...
  void onInit() {
    ...
    chequeDate = TextEditingController();
    setDate(DateTime.now());
    setCDate(DateTime.now());

    billNo = TextEditingController();
    chequeNo = TextEditingController();
  ...
  void getValue() {
    ...
    bankProvider.fetchValue(storage.read("access"), ApiConstants.value).then((response) async {
      if(response.code==1) {
  ...
  void getLastBill() {
    ...
    bankProvider.fetchLastBill(storage.read("access"), ApiConstants.bill, data).then((response) async {
      if(response.code==1) {
  ...
  void setCDate(DateTime value) {
    cdate = value;
    update();
    chequeDate.text = getDateText(cdate);
  }
  ...
  void addBank() {
    if(formKey.currentState!.validate()) {
      ...
      bankProvider.add(storage.read("access"), ApiConstants.add, data).then((
          response) async {
